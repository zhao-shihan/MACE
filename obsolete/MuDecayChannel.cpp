...
do 
{
 ...
 G4double costheta,sintheta,rphi,rtheta,rPSIMACE;
  ...
  if(G4UniformRand() > conversionP )
  {
	...
	if(elossratio != 0.)
	{
		long double bhabhaseed = G4UniformRand()*(100000.0*elossratio);
		if(bhabhaseed < 6.0)//BhaBha Scattering in the Muonium Decay
		{
			eloss = Ee*EMax*pow(10,(9.26-bhabhaseed)/1.2)/(52.318*MeV)*eV;
		
std::cout << "BhaBhaScatter! eloss = " << eloss/CLHEP::keV << std::endl;
		}
	}

  //Positron 0
  daughtermomentum[0]=sqrt((Ee*EMax-eloss)*(Ee*EMax-eloss)+2.0*(Ee*EMax-eloss)* daughtermass[0]);
 ...
 G4DynamicParticle * daughterparticle = new G4DynamicParticle (G4MT_daughters[0],	 direction0 * daughtermomentum[0]);
  ...
  G4DynamicParticle * daughterparticle3 = new G4DynamicParticle ( G4MT_daughters[3],
	 direction3 , kineticE3+eloss);
  products->PushProducts(daughterparticle3);
  }

  else 
  {
//	std::cout << "AntiMuonium Decay!" << std::endl;
    //electron 0
  ...
  products->PushProducts(daughterparticle3);
//	std::cout << "electron push!"<<std::endl;
  }
