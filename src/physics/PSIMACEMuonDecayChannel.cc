...
PSIMACEMuonDecayChannel::PSIMACEMuonDecayChannel()
                   :G4VDecayChannel()
{
}

PSIMACEMuonDecayChannel::PSIMACEMuonDecayChannel(const G4String& theParentName, 
				       G4double        theBR)
                   :G4VDecayChannel("Muon Decay",1)
{
  ...
  if (theParentName == "mu+") {
    ...
    if (GetVerboseLevel()>0) {
      G4cout << "PSIMACEMuonDecayChannel:: constructor :";
...
PSIMACEMuonDecayChannel::PSIMACEMuonDecayChannel(const PSIMACEMuonDecayChannel &right):
...
PSIMACEMuonDecayChannel & PSIMACEMuonDecayChannel::operator=(const PSIMACEMuonDecayChannel & right)
{
  if (this != &right) { 
...
G4DecayProducts *PSIMACEMuonDecayChannel::DecayIt(G4double) 
{
  // this version neglects muon polarization,and electron mass  
  //              assumes the pure V-A coupling
  //              the Neutrinos are correctly V-A. 
#ifdef G4VERBOSE
  if (GetVerboseLevel()>1) G4cout << "PSIMACEMuonDecayChannel::DecayIt ";
#endif
 ...
 //initialisation of rotation parameters
  ...
  rPSIMACE=twopi*G4UniformRand()*rad;
 ...
 // output message
  ...
  if (GetVerboseLevel()>1) {
    G4cout << "PSIMACEMuonDecayChannel::DecayIt ";
