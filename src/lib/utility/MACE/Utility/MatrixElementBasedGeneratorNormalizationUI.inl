...
auto MatrixElementBasedGeneratorNormalizationUI(const Mustard::CLI::CLI<>& cli,
    ...
    if (not biased) {
        ...
        struct Mustard::Math::IntegrationResult factor;
        const auto precisionGoal{cli->get<double>("--normalization-precision-goal")};
        if (const auto integrationState{dynamic_cast<const MatrixElementBasedGeneratorCLIModule&>(cli).ContinueNormalization()}) {
