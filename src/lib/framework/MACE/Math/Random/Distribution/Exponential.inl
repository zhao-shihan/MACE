...
MACE_STRONG_INLINE constexpr auto Exponential<T>::operator()(UniformRandomBitGenerator auto& g, const ExponentialParameter<T>& p) -> T {
    static_assert(Uniform<T>::Stateless());
    return -p.Expectation() * std::log(Uniform<T>{}(g));
}

template<std::floating_point T>
MACE_ALWAYS_INLINE constexpr auto ExponentialFast<T>::operator()(UniformRandomBitGenerator auto& g, const ExponentialFastParameter<T>& p) -> T {
    static_assert(Uniform<T>::Stateless());
