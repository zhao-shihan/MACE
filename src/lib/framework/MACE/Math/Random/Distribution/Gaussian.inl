...
MACE_STRONG_INLINE auto Gaussian<T>::operator()(UniformRandomBitGenerator auto& g, const GaussianParameter<T>& p) -> T {
    MACE_MATH_RANDOM_DISTRIBUTION_GAUSSIAN_GENERATOR_SNIPPET()
}

template<std::floating_point T>
MACE_STRONG_INLINE auto GaussianFast<T>::operator()(UniformRandomBitGenerator auto& g, const GaussianFastParameter<T>& p) -> T {
