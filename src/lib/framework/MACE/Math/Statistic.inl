...
constexpr auto Statistic<1>::Fill(double sample, double weight) -> void {
    ...
    fSumWX2 += weight * Math::Square(sample);
    fSumWX3 += weight * Math::Cubic(sample);
    fSumWX4 += weight * Math::Quartic(sample);
    fSumW += weight;
}
...
constexpr auto Statistic<1>::CentralMoment() const -> double {
    ...
    if constexpr (K == 2) { return Moment<2>() - Math::Square(Moment<1>()); }
