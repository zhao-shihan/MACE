...
auto Executor<T>::PrintExecutionSummary() const -> void {
    const auto& mpiEnv{Env::MPIEnv::Instance()};
    if (not(mpiEnv.OnCommWorldMaster() and Env::VerboseLevelReach<'E'>())) { return; }
    if (fExecuting and Env::VerboseLevelReach<'W'>()) { fmt::println(stderr, "Execution summary not available for now."); }
    fmt::println("+------------------+--------------> Summary <-------------+-------------------+\n"
...
auto Executor<T>::PostTaskReport(T iEnded) const -> void {
    ...
    const auto& mpiEnv{Env::MPIEnv::Instance()};
    if (not Env::VerboseLevelReach<'E'>()) { return; }
