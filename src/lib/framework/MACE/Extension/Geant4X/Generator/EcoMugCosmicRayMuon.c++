...
class EMLog {
public:
    enum TLogLevel { ERROR,
                     WARNING,
                     INFO,
                     DEBUG };

    enum TMsgType { EcoMug,
                    EMRandom,
                    EMMultiGen,
                    EMMaximization,
                    UNKNOWN };
...
public:
    ...
    EcoMug() :
        ...
        mGenerationTheta(0.), mGenerationPhi(0.),
    ...
    void GetAverageGenRateAndError(double& rate, double& error, int npoints = 1e7) {
        ...
        if (mGenMethod == Sky) {
            if (mCustomJ) {
                ...
                MCJprimeSkyIntegration(rate, error, npoints);
        } else if (mGenMethod == Cylinder) {
            if (mCustomJ) {
                MCJprimeCustomCylinderIntegration(rate, error, npoints);
                return;
            } else
                MCJprimeCylinderIntegration(rate, error, npoints);
        } else {
            if (mCustomJ) {
                MCJprimeCustomHSphereIntegration(rate, error, npoints);
                return;
            } else
                MCJprimeHSphereIntegration(rate, error, npoints);
        }
        rate *= k;
        error *= k;
    };
...
public:
    EMMultiGen(const EcoMug& signal, const std::vector<EcoMug>& backgrounds) :
        mIndex(-1),
        mSigInstance(signal),
        mBckInstances{backgrounds},
        mLimits(backgrounds.size() + 2),
    ...
    void Generate() {
        mIndex = (int)mDd(mRd);
        if (mIndex == 0)
            mSigInstance.Generate();
        else
            mBckInstances[mIndex - 1].Generate();
    };
...
namespace MACE::inline Extension::Geant4X::inline Generator {
