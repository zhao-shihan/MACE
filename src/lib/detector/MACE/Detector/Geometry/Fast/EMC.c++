...
void EMC::Construct(G4bool checkOverlaps) {
    ...
    const auto cellNumber = description.CellTotalNumber();
    ...
    for (G4int copyNo = 0;
        ...
        const auto MakeTessellatedSolid =
            [&,
             ...
             midCentroid = (innerRadius + outerHypotenuse) / 2 * centroid](const auto& name) {
                ...
                const auto solid = new G4TessellatedSolid(name);
                // inner surface
        ...
        const auto cutCrystalBox = new G4Box("temp", 1 * m, 1 * m, fCrystalLength / 2);
        const auto crystalTransform = Transform(-fCrystalLength / 2);
        const auto cutSoildCrystal = new G4IntersectionSolid("EMCCrystal", solidCrystal, cutCrystalBox, crystalTransform);
        //========================================== CsI(Tl) ============================================
        const auto logicCrystal = new G4LogicalVolume{cutSoildCrystal, csI, "EMCCrystal"};
        //===============================================================================================
        const auto physicalCrystal = new G4PVPlacement{{}, logicCrystal, "EMCCrystal", Mother().LogicalVolume().get(), true, copyNo, checkOverlaps};

        // Optical Photon Collection

        const auto solidOptocoupler = new G4Tubs{"temp", 0, pmtRadius, pmtCouplerThickness / 2, 0, 2 * pi};
        const auto logicOptocoupler = new G4LogicalVolume{solidOptocoupler, siliconeOil, "EMCOptocoupler"};
        const auto optocouplerTransform = Transform(pmtCouplerThickness / 2);
        const auto physicalOptocoupler = new G4PVPlacement{optocouplerTransform, logicOptocoupler, "EMCOptocoupler", Mother().LogicalVolume().get(), true, copyNo, checkOverlaps};

        const auto solidWindow = new G4Tubs{"temp", 0, pmtRadius, pmtWindowThickness / 2, 0, 2 * pi};
        const auto logicWindow = new G4LogicalVolume{solidWindow, glass, "EMCWindow"};
        const auto windowTransform = Transform(pmtCouplerThickness + pmtWindowThickness / 2);
        const auto physicalWindow = new G4PVPlacement{windowTransform, logicWindow, "EMCWindow", Mother().LogicalVolume().get(), true, copyNo, checkOverlaps};

        const auto solidCathode = new G4Tubs("temp", 0, pmtRadius, pmtCathodeThickness / 2, 0, 2 * pi);
        const auto logicCathode = new G4LogicalVolume{solidCathode, bialkali, "EMCCathode"};
        const auto cathodeTransform = Transform(pmtCouplerThickness + pmtWindowThickness + pmtCathodeThickness / 2);
        new G4PVPlacement{cathodeTransform, logicCathode, "EMCCathode", Mother().LogicalVolume().get(), true, copyNo, checkOverlaps};
