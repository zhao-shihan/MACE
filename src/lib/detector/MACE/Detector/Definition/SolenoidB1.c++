...
auto SolenoidB1::Construct(G4bool checkOverlaps) -> void {
    ...
    const auto referenceAngularSpacing{2 * std::asin(solenoid.ReferenceCoilSpacing() / (2 * solenoid.B1Radius()))};
    const auto coilAngularThickness{2 * std::asin(solenoid.CoilThickness() / (2 * solenoid.B1Radius()))};
    ...
    const auto basicTransform{G4TranslateX3D{-solenoid.B1Radius()} *
