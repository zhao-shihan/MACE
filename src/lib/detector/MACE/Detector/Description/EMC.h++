...
public:
    ...
    auto CoupleEnergyBin(std::vector<double> val) -> void { fCouplerEnergyBin = std::move(val); }
    auto CouplerRefractiveIndex(std::vector<double> val) -> void { fCouplerRefractiveIndex = std::move(val); }
