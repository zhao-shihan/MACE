...
MACEPhysicsMessenger::MACEPhysicsMessenger(MACEEmStandardPhysics* emp)
 : G4UImessenger(),
   //fDetectorConstruction(Det)
   fEmStandardPhysics(emp)
{
//	fDetectorDirectory = new G4UIdirectory("/MACE/");
//	fDetectorDirectory->SetGuidance("Detector construction control.");

	fPhysicsDirectory = new G4UIdirectory("/MACEPhysics/");
	fPhysicsDirectory->SetGuidance("EM PhysicsList control.");

	fRDPhysicsCmd = new G4UIcmdWithADouble("/MACEPhysics/SetMuonRareDecayMode",this);
	...
	fMMCPhysicsCmd = new G4UIcmdWithADouble("/MACEPhysics/SetMuMuConversion",this);
	...
	fMFPhysicsCmd = new G4UIcmdWithADouble("/MACEPhysics/SetMuFormation",this);
...
/*	fTSLengthCmd = new G4UIcmdWithADoubleAndUnit("/MACEDetector/SetTransportSysLength",this);
...
MACEPhysicsMessenger::~MACEPhysicsMessenger()
{
...
void MACEPhysicsMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
