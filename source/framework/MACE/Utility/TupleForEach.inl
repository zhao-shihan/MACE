...
class TupleForEach {
    ...
    constexpr void TupleForEachImpl(ATupleCVRef&& tuple,
                                    AFunctor&& func,
                                    std::index_sequence<AIndex...>&&) const {
        (func(std::get<AIndex>(std::forward<ATupleCVRef>(tuple))), ...);
    }

public:
    template<typename ATupleCVRef> // clang-format off
        requires (std::is_reference_v<ATupleCVRef> and std::same_as<std::remove_cvref_t<ATupleCVRef>, ATuple>)
    constexpr TupleForEach(ATupleCVRef&& tuple, AFunctor&& func) { // clang-format on
        TupleForEachImpl(std::forward<ATupleCVRef>(tuple),
                         std::forward<AFunctor>(func),
...
constexpr void TupleForEach(const AATuple<AElement...>&& tuple, AFunctor&& func) {
    Detail::TupleForEach<AATuple, AFunctor, AElement...>(tuple,
                                                         std::forward<AFunctor>(func));
}
