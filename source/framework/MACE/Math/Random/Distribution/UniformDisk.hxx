...
public:
    ...
    constexpr BasicUniformDiskParameter(VOfT r, VOfT x0, VOfT y0);
    constexpr BasicUniformDiskParameter(VOfT radius, const T& center);
    constexpr explicit BasicUniformDiskParameter(VOfT radius);
    ...
    constexpr void Radius(VOfT r) { fRadius = r; }
    constexpr void CenterX(VOfT x0) const { fCenterX = x0; }
    constexpr void CenterY(VOfT y0) const { fCenterY = y0; }
    constexpr void Center(VOfT x0, VOfT y0) { (CenterX(x0), CenterY(y0)); }
...
public:
    ...
    constexpr UniformDiskBase(VOfT r, VOfT x0, VOfT y0);
    constexpr UniformDiskBase(VOfT radius, const T& center);
    constexpr explicit UniformDiskBase(VOfT radius);
...
public:
    ...
    constexpr void Radius(VOfT r) { fParameter.Radius(r); }
    constexpr void CenterX(VOfT x0) const { fParameter.CenterX(x0); }
    constexpr void CenterY(VOfT y0) const { fParameter.CenterY(y0); }
    constexpr void Center(VOfT x0, VOfT y0) { fParameter.Center(x0, y0); }
...
protected:
    static constexpr std::pair<T, VOfT> UniformCompactDisk(UniformRandomBitGenerator auto& g, const DistributionParameter auto& p);
...
public:
    ...
    constexpr T operator()(UniformRandomBitGenerator auto& g, const UniformCompactDiskParameter<T>& p);
};
...
public:
    ...
    constexpr auto operator()(UniformRandomBitGenerator auto& g) { return (*this)(g, this->fParameter); }
    constexpr T operator()(UniformRandomBitGenerator auto& g, const UniformDiskParameter<T>& p);
};
