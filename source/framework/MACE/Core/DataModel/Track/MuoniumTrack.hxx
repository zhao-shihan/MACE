...
public:
    ...
    void SetDecayPosition(auto&&... x) requires(sizeof...(x) > 0) { Utility::AssignVector3D(fDecayPosition, std::forward<decltype(x)>(x)...); }
    void SetDecayMomentum(auto&&... p) requires(sizeof...(p) > 0) { Utility::AssignVector3D(fDecayMomentum, std::forward<decltype(p)>(p)...); }
