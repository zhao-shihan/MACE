...
bool Target::Contains(const R3VectorSpace auto& x, bool insideVolume) const noexcept {
    switch (fShape) {
    case ShapeType::Cuboid:
...
bool Target::Cuboid::VolumeContains(const R3VectorSpace auto& x) const noexcept {
    return -fThickness <= x[2] and x[2] <= 0 and
           std::abs(x[0]) <= fWidth / 2 and
           std::abs(x[1]) <= fWidth / 2;
}

bool Target::Cuboid::Contains(const R3VectorSpace auto& x, bool insideVolume) const noexcept {
    switch (fDetail) {
...
bool Target::Cuboid::TestDetectable(const R3VectorSpace auto& x) const noexcept {
...
bool Target::Cuboid::Hole::TestDetailedShape(const R3VectorSpace auto& x) const noexcept {
